...
def listOfParticipantIds(token):
    ...
    for entry in d:
        if "participant_id" in entry:
            participant_ids.append(entry["participant_id"])
...
def cleanTest(raw_test_data, default=True):
    ...
    output['participant_id'] = raw_test_data.get('participant_id')
...
def allTestsOverview(token):
    ...
    for test in d:
        overview = {}
        overview['participant_id'] = test['participant_id']
