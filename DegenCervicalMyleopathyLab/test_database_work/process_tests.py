...
def second_gai_process(d, show_plots, thresh):
    df_outdevice = f.dictListToDF(d['gai_outdevice'])
    df_restdevice = f.dictListToDF(d['gai_restdevice'])
    df_returndevice = f.dictListToDF(d['gai_returndevice'])

    start_time = datetime.strptime(d['gai_startdate'], '%Y-%m-%d %H:%M:%S')
...
def process_gai(summary_data, token, prints=False, show_plots=False, thresh=.05):
    if(type(summary_data))==list:
        data=[]
        for t in summary_data:
            data.append(process_gai(t,token,))
        data=pd.json_normalize(data)[['participant_id', 'gai_startdate', 'redcap_repeat_instance', 'm_Time of computation (unix)', 'm_Gait symmetry (percent)', 'm_Step length (m)', 'm_Step time (sec)', 'm_Step length variability (percent)', 'm_Step time variability (percent)', 'm_Step length asymmetry (percent)', 'm_Step time asymmetry (percent)', 'm_Step velocity (m/sec)', 'm_Step count lap 1', 'm_Step count lap 2']]
        data=data.rename(columns={'gai_startdate':'test_time'})
        ...
        for val in summary_data.values():
            if str(val)[0:12] == 'deviceMotion' or str(val)[0:5] == 'accel':
                field = f.dict_reverse_get(val, summary_data)
                summary_data[field] = (f.exportFile(summary_data['participant_id'], field, token,repeat_instance=repeat_instance))
...
def process_tap(cleanedTapData, prints=True, plots=False):
    if type(cleanedTapData)==list:
        ...
        data={'participant_id':participant_id,'test_time':test_time,'redcap_repeat_instance':redcap_repeat_instance}
